...
class Plugin {
    ...
    private function is_excluded() {
        ...
        if ( is_front_page() ) {
            ...
            if ( $front && in_array( $front, $ex, true ) ) return true;
        }
        if ( is_singular() ) {
            $id = get_queried_object_id();
            if ( $id && in_array( $id, $ex, true ) ) return true;
        }
        return false;
    }
    ...
    public function enqueue_assets() {
        ...
        // Font Awesome detection; load if an FA class is used and FA isn't present
        $fa_detected = wp_style_is( 'font-awesome', 'enqueued' )
            || wp_style_is( 'fontawesome', 'enqueued' )
            || wp_style_is( 'fontawesome-free', 'enqueued' )
            || wp_style_is( 'et-fontawesome', 'enqueued' )
            || wp_style_is( 'et-fontawesome-all', 'enqueued' );

        if ( ! $fa_detected && ! empty( $opts['icon_class'] ) && strpos( $opts['icon_class'], 'fa-' ) !== false ) {
    ...
    public function render_widget() {
        ...
        $icon = ! empty( $ic )
            ...
            : '<svg viewBox="0 0 24 24" class="msds-pw-svg" aria-hidden="true"><path d="M12 3c4.97 0 9 3.357 9 7.5S16.97 18 12 18c-.863 0-1.699-.099-2.49-.286L6 21l.96-3.84C5.145 15.88 3 13.46 3 10.5 3 6.357 7.03 3 12 3z"></path><rect x="11" y="7" width="2" height="7" rx="1"></rect><rect x="8" y="10" width="8" height="2" rx="1"></rect></svg>';
        ...
        <button id="msds-pw-button"
                class="msds-pw-button <?php echo esc_attr( $pos ); ?>"
                style="--msds-pw-bg:<?php echo $bg; ?>;--msds-pw-fg:<?php echo $fg; ?>;"
                aria-haspopup="dialog"
                aria-controls="msds-pw-popup"
                aria-expanded="false">
            <?php echo $icon; ?>
        </button>

        <div id="msds-pw-popup"
             class="msds-pw-popup <?php echo esc_attr( $pos ); ?>"
             hidden
             role="dialog"
             aria-modal="true"
             aria-labelledby="msds-pw-heading"
             aria-hidden="true">
            <div class="msds-pw-popup__inner">
                <button id="msds-pw-close" class="msds-pw-close" aria-label="<?php esc_attr_e( 'Close', 'msds-prayer-widget' ); ?>">×</button>
                <div class="msds-pw-popup__header">
                    <h3 id="msds-pw-heading"><?php esc_html_e( 'Prayer Request', 'msds-prayer-widget' ); ?></h3>
                </div>
                <div class="msds-pw-popup__body">
                    <?php echo $form; ?>
                </div>
            </div>
        </div>
